...
if __name__ == "__main__":
    ...
    def handle_email(email_data, phone, serial):
        try:
            ...
            # KIỂM TRA SỐ LƯỢNG, báo admin khi cần thiết
            ...
            if (count_profiles == 0):
                email_processor.send_email(os.getenv("ADMIN_MAIL"), "Hết ảnh hồ sơ", "Hết ảnh hồ sơ")
                return
            elif (count_profiles < 10):
                email_processor.send_email(os.getenv("ADMIN_MAIL"), "Số lượng hồ sơ còn lại " + str(count_profiles),
                                           "Số lượng hồ sơ còn lại " + str(count_profiles))
            # KIỂM TRA SỐ LƯỢNG HỒ SƠ
            sim = df[(df["Số điện thoại"] == phone) & (df["Serial sim"] == serial)]
            # Nếu không tìm thấy sim
            if (sim.empty):
                email_processor.reply_email(email_data, "Số serial và điện thoại không khớp!")
                return
            # Nếu sim đã kích hoạt
            if (sim["Trạng thái kích hoạt"].values[0] == "Đã kích hoạt"):
                email_processor.reply_email(email_data, "Sim đã kích hoạt!")
                return
            # Nếu sim chưa kích hoạt
            # Lấy bộ hồ sơ
            ...
            while counter > 0:
                ...
                result = sky_bot.activate_subscription(phone, serial, profile[1], profile[2], profile[3])
                ...
                if (result):
                    ...
                    df.loc[sim.index, "Thời gian kích hoạt"] = pd.Timestamp.now().strftime("%d/%m/%Y %H:%M:%S")
                    # Lưu dữ liệu
                ...
                """
                email_processor.send_email(os.getenv("ADMIN_MAIL"), "XỬ LÝ KÍCH HOẠT SIM", body_)
