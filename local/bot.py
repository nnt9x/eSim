...
class LocalBotAuto(LocalBase):
    ...
    def _login(self, url="https://api-digital.asimgroup.vn/auth-twoid-service/v2/login"):
        payload = json.dumps({
    ...
    def _get_sim_info(self, serial_number: str,
    ...
    def _activate_sim(self, url="https://api.localshop.vn/lcs-new/api/Provider/active-sim-by-enterprise-single"):
        payload = json.dumps({
    ...
    def auto_activate(self, serial_number):
        self._login()
        # self._get_sim_info(serial_number)
        # self._activate_sim()
        order = self.getOrder(serial_number, self.order_id)
        return order['phoneNumber'], order['serial'], order['status']
